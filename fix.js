...
const replacement = \{/* Skeleton / Loading state before onLoad fires */}
          {!imageLoaded && !hasError && (
            <div className=\\\w-full h-[380px] bg-slate-100 flex flex-col items-center justify-center animate-pulse p-6 text-center\\\>
              ...
              <p className=\\\	ext-sm font-medium text-slate-500\\\>Cargando estímulo visual...</p>
              <p className=\\\	ext-xs text-slate-400 mt-1\\\>El tiempo de lectura comenzará una vez visible</p>
            </div>
          )}

          {/* Headline Image Banner */}
          <div className={\\\w-full flex justify-center bg-slate-50 transition-opacity duration-300 \\\\}>
            <Image
              src={imageSrc}
              alt={stimulus.title}
              fill
